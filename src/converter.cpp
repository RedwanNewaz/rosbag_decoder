...
converter::converter(int row, int col, string &filename, string& nodename):rows(row),cols(col)
{
   video=new VideoWriter(filename,CV_FOURCC('M','J','P','G'),10, Size(cols,rows));
   file=filename;
   nh.getParam("/"+nodename+"/output/visualize",display);
}
...
can_converter::can_converter(int row, int col, string &nodename):rows(row),cols(col)
{
  ...
  prefix="/"+prefix;
  nh.getParam(prefix+"/can_bus/max_value",max_val);
  nh.getParam(prefix+"/can_bus/min_value",min_val);
  nh.getParam(prefix+"/can_bus/update",update_param);

  string can_file;
  nh.getParam(prefix+"/output/can_file",can_file);
  get_new_name(can_file,nodename);


  //ROS_INFO_STREAM("can_node: "<<can_file);

  vclass=new converter(rows,cols,can_file,nodename);
  imgCount=0;
}
...
// image converter
img_converter::img_converter(int row, int col, string &nodename):rows(row),cols(col)
{
  imgCount=0;
  string camera_file;
  nh.getParam("/"+nodename+"/output/camera_file",camera_file);
  get_new_name(camera_file,nodename);

  vclass=new converter(rows,cols,camera_file,nodename);
}
...
cloud_converter::cloud_converter(int row, int col, string &nodename):rows(row),cols(col)
{
      v_fov[0] =-30.67;
      v_fov[1] =10.67;
      v_res    =1.33;
      ...
      string cloud_file;
      nh.getParam("/"+nodename+"/output/cloud_file",cloud_file);
      get_new_name(cloud_file,nodename);

      vclass=new converter(rows,cols,cloud_file,nodename);
}
